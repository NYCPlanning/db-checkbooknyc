...
class CheckbookNYC(object):
    def __init__(self, type_name, search_criteria):
        ...
        assert self.type_name in self.type_list, "please enter one of the following type: Budget, Revenue, Contracts, Payroll, Spending. [Case Sensitive]"

    def build_xmlString(
        self, records_from: int = 1, max_records: int = 1000
    ):
        # various
        criteria = ""
        for i, j in self.search_criteria.items():
            if len(j) != 0:
                if type(j) == str:
                    ...
                    criteria += f"<criteria><name>{i}</name><type>value</type><start>{j[0]}</start><end>{j[1]}</end></criteria>"
